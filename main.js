...
var NanoBananaSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Image storage").setHeading();
    new import_obsidian.Setting(containerEl).setName("Storage mode").setDesc('Where to store generated images. "Local" saves to vault only. "Google Drive" uploads to Drive and embeds a public link (works across devices).').addDropdown(
      (dropdown) => dropdown.addOptions({
        "local": "Local (vault folder)",
        "drive": "Google Drive"
      }).setValue(this.plugin.settings.storageMode).onChange(async (value) => {
    ...
    if (this.plugin.settings.storageMode === "drive") {
      new import_obsidian.Setting(containerEl).setName("Google Client ID").setDesc("OAuth 2.0 Client ID from Google Cloud Console.").addText(
        (text) => text.setPlaceholder("your-client-id.apps.googleusercontent.com").setValue(this.plugin.settings.googleClientId).onChange(async (value) => {
          this.plugin.settings.googleClientId = value;
          await this.plugin.saveSettings();
        })
      ).addExtraButton(
        (button) => button.setIcon("external-link").setTooltip("Open Google Cloud Console").onClick(() => {
          window.open("https://console.cloud.google.com/apis/credentials");
        })
      );
      new import_obsidian.Setting(containerEl).setName("Google Client Secret").setDesc("OAuth 2.0 Client Secret from Google Cloud Console.").addText(
        (text) => text.setPlaceholder("Enter your client secret").setValue(this.plugin.settings.googleClientSecret).onChange(async (value) => {
          this.plugin.settings.googleClientSecret = value;
          await this.plugin.saveSettings();
        })
      );
      const driveService = this.plugin.getDriveService();
      const isConnected = (_a = driveService == null ? void 0 : driveService.isConnected()) != null ? _a : false;
      new import_obsidian.Setting(containerEl).setName("Google Drive authorization").setDesc(isConnected ? "Connected to Google Drive." : "Click to authorize access to Google Drive.").addButton(
        (button) => button.setButtonText(isConnected ? "Reconnect" : "Authorize").setCta().onClick(async () => {
          if (!import_obsidian.Platform.isDesktop) {
            new import_obsidian.Notice("Google Drive authorization requires the desktop app.");
            return;
          }
          if (!this.plugin.settings.googleClientId || !this.plugin.settings.googleClientSecret) {
            new import_obsidian.Notice("Please enter Client ID and Client Secret first.");
            return;
          }
          try {
            ...
            new import_obsidian.Notice(`Authorization failed: ${e instanceof Error ? e.message : String(e)}`);
          }
        })
      ).addButton(
        (button) => button.setButtonText("Test connection").onClick(async () => {
          if (!driveService || !isConnected) {
            new import_obsidian.Notice("Please authorize Google Drive first.");
            return;
          }
          new import_obsidian.Notice("Testing connection...");
          const result = await driveService.testConnection();
          if (result.connected) {
            new import_obsidian.Notice(`Connected as ${result.email || "unknown"}`);
          } else {
            new import_obsidian.Notice("Connection test failed. Please reconnect.");
          }
        })
      );
      if (isConnected) {
        new import_obsidian.Setting(containerEl).setName("Disconnect").setDesc("Remove Google Drive authorization.").addButton(
          (button) => button.setButtonText("Disconnect").setWarning().onClick(async () => {
            driveService == null ? void 0 : driveService.disconnect();
            await this.plugin.saveSettings();
      ...
      new import_obsidian.Setting(containerEl).setName("Drive folder").setDesc("Folder path in Google Drive where images will be uploaded.").addText(
        (text) => text.setPlaceholder("Obsidian/NanoBanana").setValue(this.plugin.settings.driveFolder).onChange(async (value) => {
          this.plugin.settings.driveFolder = value || "Obsidian/NanoBanana";
          await this.plugin.saveSettings();
        })
      );
      new import_obsidian.Setting(containerEl).setName("OAuth redirect port").setDesc("Local port for OAuth callback. Change only if 8586 conflicts with another app.").addText(
        (text) => text.setPlaceholder("8586").setValue(String(this.plugin.settings.oauthRedirectPort)).onChange(async (value) => {
...
var FileService = class {
   ...
   */
  async embedImageInNote(noteFile, imagePath, driveResult) {
    try {
...
<img src="https://drive.google.com/thumbnail?id=${result.fileId}&sz=w1000" alt="${result.fileName}" style="max-width: 100%; height: auto; object-fit: contain; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.15); cursor: pointer;"/>
   ...
   */
  findExistingPosterEmbed(content) {
...
var DriveService = class {
  ...
  async authorize() {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        new import_obsidian5.Notice("Please log in with Google in your browser...", 3e3);
  ...
  // ─── Upload ────────────────────────────────────────────
  async uploadImage(imageData, mimeType, fileName) {
...
\r
` + imageData + closeDelimiter;
    const uploadResponse = await this.withTimeout((0, import_obsidian5.requestUrl)({
  ...
  async testConnection() {
    ...
    try {
      const accessToken = await this.ensureValidToken();
      const response = await this.withTimeout((0, import_obsidian5.requestUrl)({
      ...
      if (response.status === 200) {
        const data = response.json;
        return { connected: true, email: (_a = data.user) == null ? void 0 : _a.emailAddress };
      }
    ...
    if (this.settings.tokenExpiresAt && this.settings.googleRefreshToken) {
      if (this.isTokenExpired(this.settings.tokenExpiresAt)) {
        ...
        try {
          ...
          new import_obsidian5.Notice("Google Drive token automatically refreshed");
        } catch (e) {
          throw new GenerationErrorClass(
            "DRIVE_UPLOAD_ERROR",
  ...
  async makeFilePublic(fileId) {
    try {
      const accessToken = await this.ensureValidToken();
      await this.withTimeout((0, import_obsidian5.requestUrl)({
...
var MESSAGES = {
  ...
  es: {
    ...
    driveUploadFailed: "Error al subir a Drive. Se insertar\xE1 la imagen local.",
    // Success
  ...
  fr: {
    ...
    // Drive
    stepUploadingDrive: "T\xE9l\xE9versement vers Google Drive",
    driveUploadFailed: "\xC9chec du t\xE9l\xE9versement Drive. Insertion de l'image locale.",
    // Success
...
var ProgressModal = class extends import_obsidian6.Modal {
  constructor(app, language = "en", storageMode = "local") {
    super(app);
    this.onCancel = null;
    ...
    if (storageMode === "drive") {
      ...
      this.steps.splice(embeddingIndex, 0, {
        key: "uploading-drive",
        label: this.messages.stepUploadingDrive,
        icon: "\u2601\uFE0F"
      });
    }
  }
...
// src/main.ts
var NanoBananaPlugin = class extends import_obsidian9.Plugin {
  constructor() {
    super(...arguments);
  ...
  async generatePoster() {
    if (this.isGenerating) {
      new import_obsidian9.Notice("Generation already in progress");
      return;
    }
    const activeView = this.app.workspace.getActiveViewOfType(import_obsidian9.MarkdownView);
    if (!activeView || !activeView.file) {
      new import_obsidian9.Notice("Please open a note first");
      return;
    }
    const noteFile = activeView.file;
    const noteContent = await this.app.vault.read(noteFile);
    if (!noteContent.trim()) {
      new import_obsidian9.Notice("Note is empty. Please add some content first.");
      return;
    }
    const providerKey = this.getProviderApiKey();
    if (!providerKey) {
      new import_obsidian9.Notice(`${this.settings.selectedProvider} API key is not configured. Please check settings.`);
      return;
    }
    if (!this.settings.googleApiKey) {
      new import_obsidian9.Notice("Google API key is required for image generation. Please configure it in settings.");
      return;
    }
...
${finalPrompt}`;
        ...
        if (this.settings.storageMode === "drive" && this.driveService) {
          ...
          try {
            ...
            new import_obsidian9.Notice(this.getProgressMessage("drive-upload-failed"));
          }
        }
        this.updateProgress(progressModal, {
          step: "embedding",
        ...
        if (progressModal) {
          ...
          new import_obsidian9.Notice("\u2705 knowledge poster generated successfully!");
        }
      }
    } catch (error) {
      ...
      if (progressModal) {
        ...
        new import_obsidian9.Notice(`\u274C generation failed: ${genError.message}`);
      }
  ...
  async generatePromptOnly() {
    ...
    const providerKey = this.getProviderApiKey();
    if (!providerKey) {
      new import_obsidian9.Notice(`${this.settings.selectedProvider} API key is not configured`);
      return;
    }
    try {
      new import_obsidian9.Notice("Generating prompt...");
      ...
      new import_obsidian9.Notice("\u2705 prompt copied to clipboard!");
    } catch (error) {
      const genError = this.toGenerationError(error);
      new import_obsidian9.Notice(`\u274C failed: ${genError.message}`);
    }
  }
  /**
  ...
  async regenerateLastPoster() {
    if (!this.lastPrompt) {
      new import_obsidian9.Notice("No previous generation found. Please generate a poster first.");
      return;
    }
    if (!this.lastNoteFile) {
      new import_obsidian9.Notice("Original note not found. Please generate a new poster.");
      return;
    }
    const file = this.app.vault.getAbstractFileByPath(this.lastNoteFile.path);
    if (!file || !(file instanceof import_obsidian9.TFile)) {
      new import_obsidian9.Notice("Original note was moved or deleted");
      return;
    }
    if (this.isGenerating) {
      new import_obsidian9.Notice("Generation already in progress");
    ...
    try {
      ...
      if (progressModal) {
        ...
        new import_obsidian9.Notice("\u2705 poster regenerated successfully!");
      }
    } catch (error) {
      const genError = this.toGenerationError(error);
      if (progressModal) {
        ...
        new import_obsidian9.Notice(`\u274C regeneration failed: ${genError.message}`);
      }
    } finally {
      this.isGenerating = false;
    }
  ...
  getProgressMessage(step) {
    ...
    const messages = {
      ...
      "uploading-drive": {
        ...
        fr: "T\xE9l\xE9versement vers Google Drive...",
      ...
      "drive-upload-failed": {
        ...
        es: "Error al subir a Drive. Se insertar\xE1 la imagen local.",
        fr: "\xC9chec du t\xE9l\xE9versement Drive. Insertion de l'image locale.",
